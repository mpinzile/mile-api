...
from app.utils.helpers import create_refresh_token_entry, generate_reset_code, hash_password, send_verification_email, success_response, error_response
...
from models.password_reset import PasswordResetHistory, PasswordResetToken
...
async def forgot_password(request: Request, db: Session = Depends(get_db)):
    ...
    identifier = body.get("identifier")
    if not identifier:
        return JSONResponse(status_code=400, content=error_response(ERROR_CODES["VALIDATION_ERROR"], "Missing identifier"))
    ...
    if user.role == AppRole.cashier:
        return JSONResponse(status_code=403, content=error_response(ERROR_CODES["FORBIDDEN"], "Cashiers cannot request password reset"))

    # Check if a recent token exists and is not used
    ...
    if existing_token:
        return JSONResponse(status_code=429, content=error_response(ERROR_CODES["TOO_MANY_REQUESTS"], "A reset code was recently sent. Please wait until it expires."))

    # Generate reset code and store hashed
    ...
    try:
        ...
        return JSONResponse(status_code=500, content=error_response(ERROR_CODES["SERVER_ERROR"], f"Failed to send reset code: {str(e)}"))

    return success_response(message="Password reset code sent to your email")
